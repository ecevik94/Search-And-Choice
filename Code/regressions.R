...
# load data
data <- read_csv("Data/subset_dt.csv")

# prepare data
names(data)
data <- data %>% 
  filter(dom == "Gain" , # only use gain data
         sampling_total_switch >= 1 # omit trials without switches
         ) %>% 
  select(# paper , # add paper variable to data set
         participant , 
         choice , 
         sampling_total , 
  ...
  mutate(# paper = as.factor(paper) , 
         ...
         choice = ifelse(choice == "A", 1, 0) , # recode to obtain numeric data
         exval = ifelse(exval == "A", 1, 0) ,
         seen_exval = ifelse(seen_exval == "A", 1, 0) ,
         seen_winner = ifelse(seen_winner == "A", 1, 0) ,
         seen_exval_choice = ifelse(choice == seen_exval, 1, 0) ,
         seen_winner_choice = ifelse(choice == seen_winner, 1, 0) ,
         I = as.factor(ifelse(seen_exval == seen_winner, 0, 1)) # indicator variable, indicating whether option with better average outcome is also option with better frequent outcome 
         )
