...
# Precise definition for winner 
```{r}

# 1- create column called round completed

cleaned_exp[, round_completed := ifelse(uniqueN(option) == 2, TRUE, FALSE), by = c("identifier", "sampling_round")]

# 2- create a column called round winner 
# it will show the winner of the completed round 

# seen_A
# count option A in a the run 
cleaned_exp <- cleaned_exp %>%  group_by(identifier, sampling_round) %>% 
...
# time to find round winner 

cleaned_exp <- cleaned_exp %>% mutate(round_seen_outcomeA_higher_prob := case_when 
                                      (round_seen_probA1 > round_seen_probA2 ~ outA1,
                                       round_seen_probA1 < round_seen_probA2 ~ outA2))

cleaned_exp <- cleaned_exp %>% mutate(round_seen_optionA_higher_prob := case_when 
                                      ( round_seen_probA1 > round_seen_probA2 ~ 'A1',
                                        round_seen_probA1 < round_seen_probA2 ~ 'A2'))

cleaned_exp <- cleaned_exp %>% mutate(round_seen_outcomeB_higher_prob := case_when 
                                      ( round_seen_probB1 > round_seen_probB2 ~ outB1,
                                        round_seen_probB1 < round_seen_probB2 ~ outB2))

cleaned_exp <- cleaned_exp %>% mutate(round_seen_optionB_higher_prob := case_when 
                                      ( round_seen_probB1 > round_seen_probB2 ~ 'B1',
                                        round_seen_probB1 < round_seen_probB2 ~ 'B2'))

cleaned_exp[, round_seen_winner := ifelse( round_seen_outcomeA_higher_prob > round_seen_outcomeB_higher_prob, "A", "B") ]


# round_winner_A
cleaned_exp <- as.data.table(cleaned_exp)

cleaned_exp[, round_winner_A := sum(round_seen_winner == "A"), by = c("identifier")]
